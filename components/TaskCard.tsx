...
export default function TaskCard({
  ...
  return (
    <div 
      className={`group relative glass-strong border p-4 md:p-7 rounded-xl transition-all duration-300 shadow-lg w-full ${
    ...
    >
      <div className="flex items-start sm:items-center gap-4 sm:gap-6 md:gap-10">
        <button 
          ...
          className={`w-12 h-12 sm:w-14 sm:h-14 rounded-lg border-2 border-white/20 flex items-center justify-center bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 hover:from-emerald-500/40 hover:to-cyan-500/40 hover:border-emerald-400/50 hover:shadow-lg hover:shadow-emerald-500/30 transition-all duration-300 flex-shrink-0 ${isConfirmingDelete ? 'opacity-20 grayscale' : ''}`}
        >
          <svg className="w-6 h-6 sm:w-7 sm:h-7 text-emerald-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex-1 min-w-0">
          {isConfirmingDelete ? (
            <div className="flex flex-col gap-3 md:gap-4">
               ...
               <div className="flex gap-3 md:gap-4">
                  <button 
                    ...
                    className="px-4 md:px-6 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:from-red-500 hover:to-red-600 transition-all shadow-lg shadow-red-500/30 hover:shadow-red-500/50"
                  >
                    Erase
                  </button>
                  <button 
                    onClick={onCancelDelete}
                    className="px-4 md:px-6 py-2 glass text-white/80 text-[9px] font-black uppercase tracking-widest rounded-lg border border-white/20 hover:bg-white/10 hover:text-white transition-all"
                  >
                    Abort
                  </button>
               </div>
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 md:gap-5 mb-2 md:mb-3">
                <h3 className="text-lg sm:text-xl font-bold truncate tracking-tight uppercase italic text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-300 group-hover:via-blue-300 group-hover:to-purple-300">{task.name}</h3>
                <span className={`text-[9px] font-black uppercase px-2 md:px-3 py-1 rounded-lg flex-shrink-0 ${
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 md:gap-8 text-[10px] font-mono">
                {minutesLeft !== null && (
        ...
        {!isConfirmingDelete && (
          <button 
            ...
            className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all p-2 sm:p-3 text-white/40 hover:text-red-400 hover:bg-red-500/10 rounded-lg flex-shrink-0"
            title="Delete Task"
          >
